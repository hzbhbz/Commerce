...
import com.digital.apigateway.PurchaseContext;
...
public class PurchaseRoute {
	...
	public ResponseEntity<?> searchOrder(@PathVariable long keyword) throws Exception {
		...
		try {
			// Purchase Info Routing
			orderRes = purchaseContext.searchPurchase(keyword);
		} catch (Exception e) {
	...
	public ResponseEntity<?> purchase(@Parameter(name = "결제", required = true) @RequestBody Purchase purchase, HttpServletRequest request) {
		...
		try {
			if (purchaseContext.insertPurchase(purchase) != null) {
				success.setSuccessCode(200);
				success.setSuccessMsg("결제가 완료되었습니다.");
			}
		} catch (Exception e) {
			return ExceptionUtils.setException(errors, 500, e.getMessage(), header);
		}

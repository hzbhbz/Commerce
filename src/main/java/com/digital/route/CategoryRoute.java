...
import com.digital.apigateway.CategoryContext;
...
public class CategoryRoute {
	...
	public ResponseEntity<?> productList(@PathVariable String keyword) {
		...
		Category categoryRes = new Category();
		ErrorMsg errors = new ErrorMsg();
		try {
			categoryRes = categoryContext.searchCategory(keyword);
		} catch (Exception e) {
			return ExceptionUtils.setException(errors, 500, e.getMessage(), header);
		}
		return new ResponseEntity<Category>(categoryRes, header, HttpStatus.valueOf(200)); // ResponseEntity를 활용한 응답 생성
	}
	...
	public ResponseEntity<?> categoryWrite(
		...
		Category categoryRes = new Category();
		try {
			// Category Set Routing
			categoryRes = categoryContext.insertCategory(category);
